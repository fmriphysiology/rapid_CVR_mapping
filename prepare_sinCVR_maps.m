function bids=prepare_sinCVR_maps(bids,subj)
	...
	[cope dims scales bpp endian]=read_avw([bids(subj).func(1).analysis(2).feat 'stats/cope1']);
	...
	freq=find(f>(1/60),1,'first');
	cvr8_mag=reshape(abs(H(freq,:)),func_dims(1),func_dims(2),func_dims(3))./mean_func;
	cvr8_delay=reshape(atan(real(H(freq,:))./imag(H(freq,:))),func_dims(1),func_dims(2),func_dims(3));
	cvr8_delay(cvr8_delay<0)=cvr8_delay(cvr8_delay<0)+pi;
	
	bids(subj).func(1).results(8).name='cvr magnitude/delay maps from sinCVR - transfer function analysis petco2 regressor';
	s=regexp(bids(subj).func(1).fname,'/');
	bids(subj).func(1).results(8).cvr_mag=[bids(subj).dir 'derivatives/' bids(subj).name '/results/' bids(subj).func(1).fname(s(end)+1:end) '_cvr8-mag'];
	save_avw(cvr8_mag,bids(subj).func(1).results(8).cvr_mag,'f',scales');
	
	bids(subj).func(1).results(8).cvr_delay=[bids(subj).dir 'derivatives/' bids(subj).name '/results/' bids(subj).func(1).fname(s(end)+1:end) '_cvr8-delay'];
	save_avw(cvr8_delay,bids(subj).func(1).results(8).cvr_delay,'f',scales');	
